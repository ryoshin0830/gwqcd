...
The emitted function shares its name with the binary, so `eval "$(gwqcd --init
...
already defined. The npx form (`eval "$(npx -y gwqcd --init zsh)"`) never had the
...
Trusted Publisher: GitHub Actions, owner `ryoshin0830`, repository `gwqcd`,
